...
Akka Streams is a powerful streaming library that allows developers to manipulate streaming data in multiple and genuine ways. On the other hand, it is not providing aggregations of the data like you can find in other Streaming libraries like Spark or Kafka Streams.

Aggregating data is one of the most common problems that streaming applications raise. Akka streams is a low level library that allows developers to do great stuff and manipulate the incoming data in such amazing ways. The main difference between Akka Streams and **Apache Spark** is that Spark is a higher level library that includes more boundaries and limits what the developer can do.
...
But, what exactly is streaming windowing?
...
But in this case, the first question is, *"Ok but, how do I get the event time of a record consumed from the stream?"* . There are two ways:
...
* **Processing time**: the event time is acquired from the system timestamp of the akka streams application.

The processing time based windows is not exactly what we want, as the incoming records are unordered. It could happen that we are processing a record that happened 5 minutes ago, so using the processing time windowing as a mechanism to attach a record to a window is not ideal.
...
* *Sliding Windows*: the windows overlap one another depending of the **step time**.  One record can be part of different windows at the same time. For instance we can have a window with length of 30 minutes and steps of 10 minutes, these would mean that for a time based starting from 00:00 our windows would be: W1(00:00 -00:30), W2(00:10 - 00:40), W3 (00:20 - 00:50) ...
* *Tumbling Windows*: there is no overlap. Every record belongs just to one window. For instance for a window with length of 30 minutes, the windows associated started from 00:00 would be: W1(00:00 - 00:30), W2(00:30,01:00), W3(01:00 - 01:30) ...
...
As we commented briefly before, the records do not come ordered. Watermarking is a techique that deals with lateness. It is the threshold that tells the system how long it has to wait for late records.
...
As you can notice the record itself contains the event time. Good!

We want to implement a window solution with the following features:
* Tumbling windows: this means that the window step and window length is the same value. There will be no overlap between windows. Every record would be part of the only one window.
* Watermark: 5 seconds.
...
Before we continue, let's recap quickly the different Akka Stream components:
* **Source**: a processing stage that contains exactly one output. This is the beginning of the streaming app.
...
After this quick recap, let's focus on our example. Now comes the complicated part: creating the windows and assigning events to them. In our case we want one a tumbling window, so it means that the windows do not overlap with each other.
